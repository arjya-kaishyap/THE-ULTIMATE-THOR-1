...
function setup() {
	...
	//creating sprites for powerUp coins
...
function draw() {
	...
	//creating gamestate from wait to gamestate play
	...
	//spawning power
	  ...
	  //moving thor up down right left from "d","a","s","w"
	   if(keyDown("d")){
		   thor1.x += 20;
		 }
		 if(keyDown("a")){
	  ...
	  drawSprites();
				...
				//calling function fight and function leaderfight
				fight()
				leaderfight()
				//if life is 0 then gamestate will be over
				if(life <= 0){
					gameState = "over"
		...
		//creating if thor win then "you win" text
		if(gameState === "win"){
